 ...
 * @version $Id$
 */
public abstract class PTRS_ProblemProcessor extends Processor.ProcessorSkeleton {
    ...
    public boolean isApplicable(final BasicObligation o) {
        if (o instanceof final PTRSProblem R) {
            if (R.getPR().isEmpty()) {
                return true;
            } else {
                return isPTRSApplicable(R);
            }
        }
        return false;
    }
    ...
    public Result process(final BasicObligation o, final BasicObligationNode oblNode, final Abortion aborter, final RuntimeInformation rti)
        ...
        if (BasicObligationCache.oblCache != null) {
            ...
            if (value != null) {
                switch (value) {
                    case YES:
                        return ResultFactory.proved(PTRS_ProblemProcessor.pIsCachedProof);
                    case NO:
                        return ResultFactory.disproved(PTRS_ProblemProcessor.pIsCachedProof);
                    default:
                        break;
                }
            }
    ...
    private static final class RisEmptyProof extends Proof {
        ...
        private ProbabilisticTerminationResult target;
        ...
        public String export(final Export_Util o, final VerbosityLevel level) {
            ...
            res.append(" ");
            switch (this.target) {
                case certainTermination -> res.append("termination");
                case AST -> res.append("AST");
                case SAST -> res.append("SAST");
                default -> res.append("termination");
            }
            res.append(" is trivially proven.");
