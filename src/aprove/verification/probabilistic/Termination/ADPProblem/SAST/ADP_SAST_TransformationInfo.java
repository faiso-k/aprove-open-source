 ...
 * when using DP transformation for SAST of Narrowing, Rewriting, Instantiations, ...
 *
 ...
 * @version $Id$
 */
public class ADP_SAST_TransformationInfo {
    ...
    private static final int SizeOfArray = ADP_SAST_TransformationInfo.NrOfTransformations + 1; // +1 for the origin.

    private static final int ORIGIN_DUMMY = -1;
    private int origins = ADP_SAST_TransformationInfo.ORIGIN_DUMMY; // the nr of original dts addressed in the map

    private static final int getIndex(final ADP_SAST_Transformation transformation) {
        ...
        throw new RuntimeException("Unknown Transformation in SAST_ADPTransformationInfo");
    }
    ...
    private static Object[] getCopy(final Object[] info) {
        final Object[] res = new Object[ADP_SAST_TransformationInfo.SizeOfArray];
        System.arraycopy(info, 0, res, 0, ADP_SAST_TransformationInfo.SizeOfArray);
        return res;
    }
     ...
     */
    public ADP_SAST_TransformationInfo(final Set<Node<ProbabilisticRule>> nodes) {
     ...
     */
    private ADP_SAST_TransformationInfo(final Set<Node<ProbabilisticRule>> subset, final ADP_SAST_TransformationInfo superInformation) {
     ...
     * @return
     */
    private ADP_SAST_TransformationInfo(final ADP_SAST_Transformation transformation,
        final Node<ProbabilisticRule> origNode,
        final Set<Node<ProbabilisticRule>> newDPs,
        final Position p,
        final ADP_SAST_TransformationInfo superInformation) {
        this.map = new LinkedHashMap<>(superInformation.map);
        Object[] info = this.map.remove(origNode);
        info = ADP_SAST_TransformationInfo.getCopy(info);
        final int index = ADP_SAST_TransformationInfo.getIndex(transformation);

        Object o = info[index];

        // this code has to be specified for each transformation
        if (transformation == ADP_SAST_Transformation.Narrowing) {
            final PositionTree narrowInfo = (PositionTree) o;
     ...
     * @param p - the position
     */
    public Pair<ADP_SAST_TransformationInfo, Integer>
        getTransformedInfo(final ADP_SAST_Transformation transformation,
            final Node<ProbabilisticRule> origNode,
            final Set<Node<ProbabilisticRule>> newDPs,
            final Position p) {
        final ADP_SAST_TransformationInfo transInfo = new ADP_SAST_TransformationInfo(transformation, origNode, newDPs, p, this);
        int limit;
        if (newDPs.isEmpty()) {
            limit = 0;
        } else {
     ...
     */
    public ADP_SAST_TransformationInfo getSubInfo(final Set<Node<ProbabilisticRule>> subset) {
        return new ADP_SAST_TransformationInfo(subset, this);
    }
    ...
    public int getNrOfOrigins() {
        if (this.origins == ADP_SAST_TransformationInfo.ORIGIN_DUMMY) {
            ...
            for (final Object[] elem : this.map.values()) {
                origins.add((ProbabilisticRule) elem[ADP_SAST_TransformationInfo.NrOfTransformations]);
            }
     ...
     */
    public int getCount(final ADP_SAST_Transformation transformation, final Node<ProbabilisticRule> node) {
        final Object o = this.map.get(node)[ADP_SAST_TransformationInfo.getIndex(transformation)];
        if (transformation == ADP_SAST_Transformation.Narrowing) {
    ...
    private final static class PositionTree {
        ...
        public PositionTree addPosition(final Position p) {
            return addPos(p.iterator());
        }

        private PositionTree addPos(final Iterator<Integer> p) {
            if (p.hasNext()) {
                final int firstPos = p.next();
                ...
                final Map<Integer, PositionTree> newChildren = new HashMap<>(this.children);
