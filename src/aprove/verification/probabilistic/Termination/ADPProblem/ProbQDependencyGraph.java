 ...
 * Instantiation, ... processors (DT transformations). This is
 ...
 * @version $Id$
 */
public class ProbQDependencyGraph implements Immutable {
    ...
    private final ADP_AST_TransformationInfo transformationInfo; // values for heuristics of NRI-transformations.

    /* computed values */
    private final QTermSet Q;
    private final boolean QsuperR;
    private final Set<Cycle<ProbabilisticRule>> sccs; // the set of sccs of this graph, null if the graph itself is an SCC;
    // must be computed in the constructor

    //Map to store the usable terms for each DT, DTs stored in STD representation
    //Only computed for edges on SCCs.
     ...
     * @param sWithQ
     */
    private ProbQDependencyGraph(final Set<ProbabilisticRule> P,
        ...
        if (graph == null) {
            ...
            this.transformationInfo = new ADP_AST_TransformationInfo(nodes);
            ...
            this.transformationInfo = new ADP_AST_TransformationInfo(graph.getNodes());
            this.g = graph;
        ...
        final Triple<Set<Cycle<ProbabilisticRule>>, Map<TRSFunctionApplication, Set<Triple<ProbabilisticRule, TRSSubstitution, TRSSubstitution>>>, Map<ProbabilisticRule, Set<TRSFunctionApplication>>> sccsAndUsableTermsMaps =
            checkEdgesOnSccs(computeSccs());
        this.sccs = sccsAndUsableTermsMaps.x;
        this.usableTermsToSuccessorWithMGU = sccsAndUsableTermsMaps.y;
        this.dtToUsableTerms = sccsAndUsableTermsMaps.z;
    }
     ...
     * @param superGraph
     */
    private ProbQDependencyGraph(final Set<Node<ProbabilisticRule>> P, final ProbQDependencyGraph superGraph) {
        if (Globals.useAssertions) {
     ...
     * @param superGraph
     */
    private ProbQDependencyGraph(final Graph<ProbabilisticRule, ?> P,
        final ADP_AST_TransformationInfo transformationInfo,
        final ProbQDependencyGraph superGraph,
        final Set<ProbabilisticRule> newRules) {
        this.transformationInfo = transformationInfo;
        this.annoMap = superGraph.annoMap;

        final PQTRSProblem oldPQTRS = superGraph.capCalculator.getUnderlyingPQTRS();
        final Set<ProbabilisticRule> newPR = new HashSet<>();
        newPR.addAll(oldPQTRS.getPR());
        newPR.addAll(newRules);
        final PQTRSProblem newPQTRS =
            PQTRSProblem.create(ImmutableCreator.create(newPR), oldPQTRS.getQ(), oldPQTRS.getStrat(), oldPQTRS.getTarget(), oldPQTRS.isBasic());
    ...
    private ProbQDependencyGraph(
        ...
        for (final Cycle<ProbabilisticRule> cycle : superGraph.sccs) {
            this.sccs.add(cycle);
        }

        final Pair<Map<TRSFunctionApplication, Set<Triple<ProbabilisticRule, TRSSubstitution, TRSSubstitution>>>, Map<ProbabilisticRule, Set<TRSFunctionApplication>>> sccsAndUsableTermsMaps =
            computeUsableTermsMappings(computeSccs());
        this.usableTermsToSuccessorWithMGU = sccsAndUsableTermsMaps.x;
        this.dtToUsableTerms = sccsAndUsableTermsMaps.y;
    }
     ...
     * @param superGraph
     */
    private ProbQDependencyGraph(final Set<Node<ProbabilisticRule>> P,
        final PQTRSProblem sWithQ,
        final ProbQDependencyGraph superGraph) {
        if (Globals.useAssertions) {
            assert (superGraph.g.getNodes().containsAll(P));
     ...
     * @param superGraph
     */
    private ProbQDependencyGraph(final PQTRSProblem sWithQ, final ProbQDependencyGraph superGraph) {
    ...
    public ProbQDependencyGraph(final Graph<ProbabilisticRule, ?> P,
        final ADP_AST_TransformationInfo transformationInfo,
        final ProbQDependencyGraph superGraph,
        ...
        final Triple<Set<Cycle<ProbabilisticRule>>, Map<TRSFunctionApplication, Set<Triple<ProbabilisticRule, TRSSubstitution, TRSSubstitution>>>, Map<ProbabilisticRule, Set<TRSFunctionApplication>>> sccsAndUsableTermsMaps =
            checkEdgesOnSccs(computeSccs());
        this.sccs = sccsAndUsableTermsMaps.x;
        this.usableTermsToSuccessorWithMGU = usableTermsToSuccessorsWithMGU;
        this.dtToUsableTerms = dtToUsableTerms2;
    }
     ...
     * @return
     */
    public static ProbQDependencyGraph
        create(final Graph<ProbabilisticRule, ?> P, final PQTRSProblem sWithQ, final BidirectionalMap<FunctionSymbol, FunctionSymbol> annoMap) {
        return new ProbQDependencyGraph(null, sWithQ, P, annoMap);
    }
     ...
     */
    public static ProbQDependencyGraph
        create(final Set<ProbabilisticRule> P, final PQTRSProblem sWithQ, final BidirectionalMap<FunctionSymbol, FunctionSymbol> annoMap) {
        return new ProbQDependencyGraph(P, sWithQ, null, annoMap);
    }
     ...
     */
    public ADP_AST_TransformationInfo getTransformationInfo() {
     ...
     * @return
     */
    public ProbQDependencyGraph getSubGraphByDroppingEdges(
        final Collection<Pair<Node<ProbabilisticRule>, Node<ProbabilisticRule>>> dropTheseEdges) {
        return new ProbQDependencyGraph(dropTheseEdges, this);
    }

    public ProbQDependencyGraph getSubGraph(final Set<Node<ProbabilisticRule>> P) {
        return new ProbQDependencyGraph(P, this);
    }

    public ProbQDependencyGraph getSubGraphFromPRules(final Set<ProbabilisticRule> P) {
        final Set<Node<ProbabilisticRule>> nodesForP = this.g.getNodesFromObjects(P);
        return new ProbQDependencyGraph(nodesForP, this);
    }
     ...
     */
    public ProbQDependencyGraph getSubGraph(final Set<ProbabilisticRule> P, final PQTRSProblem sWithQ) {
        final Set<Node<ProbabilisticRule>> nodesForP = this.g.getNodesFromObjects(P);
        return new ProbQDependencyGraph(nodesForP, sWithQ, this);
    }
     ...
     * @return
     */
    public ProbQDependencyGraph getUsableRulesSubGraph(final PQTRSProblem sWithQ) {
        return new ProbQDependencyGraph(sWithQ, this);
    }
    ...
    public boolean isGenuineSCC() {
        final Set<Cycle<ProbabilisticRule>> sccs = this.g.getSCCs();
        return sccs.size() > 0;
    }
     ...
     * @return
     */
    public Set<ProbQDependencyGraph> getSubSCCs() {
        final Set<ProbQDependencyGraph> subSccs = new LinkedHashSet<>(this.sccs.size());
        for (final Set<Node<ProbabilisticRule>> scc : this.sccs) {
     ...
     * @return
     */
    public Set<ProbQDependencyGraph> getSubSCCs(final boolean onlyReal) {
        final Set<ProbQDependencyGraph> subSccs = new LinkedHashSet<>(
            this.sccs.size());
        for (final Set<Node<ProbabilisticRule>> scc : this.g.getSCCs(onlyReal)) {
            subSccs.add(this.getSubGraph(scc));
        }
        return subSccs;
    }
     ...
     */
    public Set<Node<ProbabilisticRule>> getNodesOnSCCs() {
        if (this.sccs == null) {
            return this.g.getNodes();
        } else {
            final Set<Node<ProbabilisticRule>> nodes = new HashSet<>(
     ...
     */
    public Pair<ProbQDependencyGraph, Integer>
        getTransformedGraph(final ADP_AST_Transformation transformation,
        ...
        final Pair<ADP_AST_TransformationInfo, Integer> transInfoCounter =
            this.transformationInfo.getTransformedInfo(transformation, s_to_t, reallyNewNodes, p);

        return new Pair<>(new ProbQDependencyGraph(newGraph, transInfoCounter.x, this, newRules), transInfoCounter.y);
    }
    ...
    private Set<Cycle<ProbabilisticRule>> computeSccs() {
        final Set<Cycle<ProbabilisticRule>> sccs = this.g
            .getSCCs(!((Options.certifier.isRainbow()
                || Options.certifier.isCeta()
                || Options.certifier.isA3pat()
                || Options.certifier
                    .isCpf())));
        if (sccs.size() == 1) {
     ...
     */
    private
        Triple<Set<Cycle<ProbabilisticRule>>, Map<TRSFunctionApplication, Set<Triple<ProbabilisticRule, TRSSubstitution, TRSSubstitution>>>, Map<ProbabilisticRule, Set<TRSFunctionApplication>>>
        checkEdgesOnSccs(
            final Set<Cycle<ProbabilisticRule>> localSccs) {

        Set<Cycle<ProbabilisticRule>> newLocalSccs = localSccs;
        ...
        List<Edge<?, ProbabilisticRule>> edges = new ArrayList<>(this.g.getEdges());
        boolean changed = false;
        if (newLocalSccs == null) {
            ...
            final Map<Node<ProbabilisticRule>, Cycle<ProbabilisticRule>> nodeToScc = new HashMap<>(
                this.g.getNodes().size());
            for (final Cycle<ProbabilisticRule> cycle : newLocalSccs) {
                for (final Node<ProbabilisticRule> node : cycle) {
            ...
            for (final Edge<?, ProbabilisticRule> e : edges) {
                final Node<ProbabilisticRule> from = e.getStartNode();
                final Node<ProbabilisticRule> to = e.getEndNode();
                final Cycle<ProbabilisticRule> fromCycle = nodeToScc.get(from);
                if (fromCycle != null && fromCycle == nodeToScc.get(to)) {
                    final Set<Pair<TRSFunctionApplication, TRSSubstitution>> connectionSet =
                        calculateConnectionWithUsableTerms(from.getObject(), to.getObject());
                    if (connectionSet.isEmpty()) {
                        this.g.removeEdge(from, to);
                        changed = true;
                    } else {
        ...
        if (changed) {
            newLocalSccs = computeSccs();
        }

        // now check *-graph conditions
        final PQTRSProblem qtrs = this.capCalculator.getUnderlyingPQTRS();
        if (this.QsuperR || !qtrs.isCollapsing()) {
            ...
            edges = new ArrayList<>(this.g.getEdges());
            changed = false;
            if (newLocalSccs == null) {
                for (final Edge<?, ProbabilisticRule> e : edges) {
                    final Node<ProbabilisticRule> from = e.getStartNode();
        ...
        return new Triple<>(newLocalSccs, usableTermsToSuccessors, dtToUsableTerms);
    }

    /**
     * Computes the usable terms mappings on sccs. Assumes that we have called checkEdgesOnSccs beforehand.
     * Does not change the tree structure! Only computes mappings.
     *
     * @param localSccs - the set of local Sccs. May not be null!
     */
    private
        ...
        computeUsableTermsMappings(final Set<Cycle<ProbabilisticRule>> localSccs) {

        final Set<Cycle<ProbabilisticRule>> newLocalSccs = localSccs;
        ...
        // check normal graph conditions
        for (final Edge<?, ProbabilisticRule> e : edges) {
            final Node<ProbabilisticRule> from = e.getStartNode();
            final Node<ProbabilisticRule> to = e.getEndNode();
            final Cycle<ProbabilisticRule> fromCycle = nodeToScc.get(from);
            if (fromCycle != null && fromCycle == nodeToScc.get(to)) {
                final Set<Pair<TRSFunctionApplication, TRSSubstitution>> connectionSet = calculateConnectionWithUsableTerms(from.getObject(), to.getObject());
                if (!connectionSet.isEmpty()) {
                    //Update the dtToUsableTerms map
                    final Set<TRSFunctionApplication> usableForDT = dtToUsableTerms.get(from.getObject().getStandardRepresentation());
                    for (final Pair<TRSFunctionApplication, TRSSubstitution> pair : connectionSet) {
