 ...
 * @version $Id$
 */
public class ADP_Cpx_TransformationInfo {
    ...
    private static final int SizeOfArray = ADP_Cpx_TransformationInfo.NrOfTransformations + 1; // +1 for the origin.

    private static final int ORIGIN_DUMMY = -1;
    private int origins = ADP_Cpx_TransformationInfo.ORIGIN_DUMMY; // the nr of original dts addressed in the map

    private static final int getIndex(final ADP_Cpx_Transformation transformation) {
    ...
    private static Object[] getCopy(final Object[] info) {
        final Object[] res = new Object[ADP_Cpx_TransformationInfo.SizeOfArray];
        System.arraycopy(info, 0, res, 0, ADP_Cpx_TransformationInfo.SizeOfArray);
        return res;
    }
     ...
     */
    public ADP_Cpx_TransformationInfo(final Set<Node<ProbabilisticRule>> nodes) {
     ...
     */
    private ADP_Cpx_TransformationInfo(final Set<Node<ProbabilisticRule>> subset, final ADP_Cpx_TransformationInfo superInformation) {
     ...
     * @return
     */
    private ADP_Cpx_TransformationInfo(final ADP_Cpx_Transformation transformation,
        final Node<ProbabilisticRule> origNode,
        final Set<Node<ProbabilisticRule>> newDPs,
        final Position p,
        final ADP_Cpx_TransformationInfo superInformation) {
        this.map = new LinkedHashMap<>(superInformation.map);
        Object[] info = this.map.remove(origNode);
        info = ADP_Cpx_TransformationInfo.getCopy(info);
        final int index = ADP_Cpx_TransformationInfo.getIndex(transformation);

        Object o = info[index];

        // this code has to be specified for each transformation
        if (transformation == ADP_Cpx_Transformation.Narrowing) {
     ...
     * @param p - the position
     */
    public Pair<ADP_Cpx_TransformationInfo, Integer>
        getTransformedInfo(final ADP_Cpx_Transformation transformation,
            final Node<ProbabilisticRule> origNode,
            final Set<Node<ProbabilisticRule>> newDPs,
            final Position p) {
        final ADP_Cpx_TransformationInfo transInfo = new ADP_Cpx_TransformationInfo(transformation, origNode, newDPs, p, this);
        int limit;
        if (newDPs.isEmpty()) {
            limit = 0;
        } else {
     ...
     */
    public ADP_Cpx_TransformationInfo getSubInfo(final Set<Node<ProbabilisticRule>> subset) {
        return new ADP_Cpx_TransformationInfo(subset, this);
    }
    ...
    public int getNrOfOrigins() {
        if (this.origins == ADP_Cpx_TransformationInfo.ORIGIN_DUMMY) {
            ...
            for (final Object[] elem : this.map.values()) {
                origins.add((ProbabilisticRule) elem[ADP_Cpx_TransformationInfo.NrOfTransformations]);
            }
     ...
     */
    public int getCount(final ADP_Cpx_Transformation transformation, final Node<ProbabilisticRule> node) {
        final Object o = this.map.get(node)[ADP_Cpx_TransformationInfo.getIndex(transformation)];
        if (transformation == ADP_Cpx_Transformation.Narrowing) {
