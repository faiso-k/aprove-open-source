...
public class IcapCalculator {
     ...
     */
    private final ConcurrentHashMap<Cdt, List<TRSTerm>> capCache;
    ...
    public IcapCalculator(Set<Rule> rules) {
        this.capCache = new ConcurrentHashMap<Cdt, List<TRSTerm>>();
    ...
    public IcapCalculator(IcapCalculator oldCalc) {
        this.capCache = new ConcurrentHashMap<Cdt, List<TRSTerm>>(oldCalc.capCache);
     ...
     * or getCappedRhs.
     */
    public Cdt renameVarDisjoint(Cdt cdt) {
        return Cdt.create(cdt.getRule().getWithRenumberedVariables(IcapAlgorithm.PREFIX_NOTCAP));
    }

    public Cdt renameWithCapInput(Cdt cdt) {
        return Cdt.create(cdt.getRule().getWithRenumberedVariables(IcapAlgorithm.PREFIX_CAP_INPUT));
    }

    public TRSTerm getCappedLhs(TRSFunctionApplication precedingCdtLhs, TRSTerm precedingCdtRhsArg, Cdt toBeCapped) {
        CacheKey cacheKey =
    ...
    public List<TRSTerm> getCappedRhs(Cdt cdt) {
    ...
    private ImmutableRuleSet<GeneralizedRule> reverseRules(
            Set<Rule> renumberedRules) {
