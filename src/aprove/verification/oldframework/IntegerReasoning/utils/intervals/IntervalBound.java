 ...
 * @author cotto
 */
final class IntervalBound implements Immutable {
    /**
     * One!
     */
    ...
    public IntervalBound subtract(final IntervalBound subtrahend) {
        assert this.constant != null || !this.posInf;
        assert subtrahend.constant != null || subtrahend.posInf;

        if (this.constant != null && subtrahend.constant != null) {
            return new IntervalBound(this.constant.subtract(subtrahend.constant));
        } else {
            // Either this is -inf or the subtrahend is +inf. In both cases,
            // the result is -inf
    ...
    /**
     * Do not compare two infinites of the same sign!
     * @param arg the number to compare with
     * @return -1, 0, 1 if this number is smaller / equal / bigger than the
     * argument
     */
