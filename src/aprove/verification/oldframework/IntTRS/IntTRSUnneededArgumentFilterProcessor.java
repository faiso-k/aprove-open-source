 ...
 */
public class IntTRSUnneededArgumentFilterProcessor extends IntTRSArgumentFilter {
    @Override
        ...
        final RuntimeInformation rti) throws AbortionException
    {
        ...
        final ArgumentFilterResult r =
            IntTRSUnneededArgumentFilterProcessor.processRules(intTRS.getRules(), false, oldNewMap);
        ...
        if (r != null) {
            ...
            return ResultFactory.proved(intTRS.create(r.x.x, newStartTerm),
                    YNMImplication.EQUIVALENT,
                    new IntTRSUnneededArgumentFilterProof(ArgumentsRemovalProof.getFilterRules(r.y, r.x.y), oldNewMap, r));
        } else {
     ...
     */
    public static ArgumentFilterResult processRules(final Set<IGeneralizedRule> rules, boolean allowOverapproximation, Map<IGeneralizedRule, IGeneralizedRule> oldNewMap) {
        return IntTRSUnneededArgumentFilterProcessor.processRules(rules, java.util.Collections.<FunctionSymbol>emptySet(), allowOverapproximation, oldNewMap);
    }
    ...
    public static ArgumentFilterResult processRules(
        ...
        final Pair<CollectionMap<FunctionSymbol, FunctionSymbol>, Map<FunctionSymbol, CollectionMap<Integer, TRSTerm>>> p =
            IntTRSUnneededArgumentFilterProcessor.init(rules);
        ...
        do {
            ...
            for (final IGeneralizedRule rule : rules) {
                final Collection<TRSVariable> neededVars =
                    IntTRSUnneededArgumentFilterProcessor.getNeededVariablesInFilteredTerm(rule.getRight(), newNeededPositions);
                if (rule.getCondTerm() != null) {
                    neededVars.addAll(rule.getCondVariables());
                }
                final LinkedList<TRSFunctionApplication> lhsSubTermsToCheck = new LinkedList<TRSFunctionApplication>();
                            ...
                            if (neededVars.contains(arg)) {
                                ...
                                for (int otherPos = 0; otherPos < faArgs.size(); otherPos++) {
                                    final Collection<TRSVariable> otherPosVariables =
                                        IntTRSUnneededArgumentFilterProcessor.getNeededVariablesInFilteredTerm(faArgs.get(otherPos), newNeededPositions);
                            ...
                            .equals(IntegerConstraintCleaner.INTERNAL_MAX_SYMBOL))
                    {
                        ...
                        nextPos: for (int pos = 0; pos < args.size(); pos++) {
                            ...
                            for (final FunctionSymbol sym : arg.getFunctionSymbols()) {
                                if (IntTRSUnneededArgumentFilterProcessor.isPredefinedRelation(sym)) {
                                    if (newNeededPositions.add(fs, pos)) {
                                        changed = true;
                                    }
                                    continue nextPos;
        ...
        if (!allowOverapproximation) {
            for (final IGeneralizedRule rule : rules) {
                dontRemovePositionsNeededForNeededConditions(rule, rule.getCondTerm(), positionsToBeRemoved, IDPPredefinedMap.DEFAULT_MAP);
            }
        }
        ...
        return new ArgumentFilterResult(newRulePair, positionsToBeRemoved);
    }
     ...
     */
    private static boolean dontRemovePositionsNeededForNeededConditions(IGeneralizedRule rule, TRSTerm condTerm, CollectionMap<FunctionSymbol, Integer> positionsToBeRemoved, IDPPredefinedMap predefinedMap) {
        ...
        funcAppls.add((TRSFunctionApplication)rule.getRight());
    ...
    private static
        ...
        init(final Set<IGeneralizedRule> rules)
    {
        final Collection<FunctionSymbol> definedSymbols = new LinkedHashSet<FunctionSymbol>();
        ...
        for (final IGeneralizedRule rule : rules) {
            for (final TRSTerm t : rule.getLeft().getSubTerms()) {
                if (t instanceof TRSFunctionApplication) {
                    ...
                    IntTRSUnneededArgumentFilterProcessor.noteTermsForFS(fa, termsAtFsPosition);
                }
            }
        ...
        return new Pair<CollectionMap<FunctionSymbol, FunctionSymbol>, Map<FunctionSymbol, CollectionMap<Integer, TRSTerm>>>(
            possiblyReaching,
            termsAtFsPosition);
    }
    ...
    private static Collection<TRSVariable> getNeededVariablesInFilteredTerm(
        ...
        for (int faPos = 0; faPos < args.size(); faPos++) {
            ...
            {
                neededVars.addAll(IntTRSUnneededArgumentFilterProcessor.getNeededVariablesInFilteredTerm(args.get(faPos), keptPositions));
            }
        }
        return neededVars;
    }
     ...
     * @author Marc Brockschmidt
     */
    private class IntTRSUnneededArgumentFilterProof extends DefaultProof {
        /**
        ...
        private final ArgumentFilterResult filter;
        
        /**
         * Create a new proof.
         * @param removedArgs maps function symbols to the list of removed argument positions
         */
        public IntTRSUnneededArgumentFilterProof(final Collection<Rule> removedArgs, Map<IGeneralizedRule, IGeneralizedRule> oldNewMap,
        	ArgumentFilterResult filter) {
        ...
        public XMLMetaData adaptMetaData(final XMLMetaData xmlPreMetaData) {
            XMLMetaData midData = xmlPreMetaData.adjustOldNew(this.oldNewMap);
            return midData.integrateFilter(this.filter);
        }
