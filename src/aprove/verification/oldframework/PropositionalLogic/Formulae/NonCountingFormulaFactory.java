...
public class NonCountingFormulaFactory<T> extends AbstractFormulaFactory<T> {
    ...
    private static final Logger log = Logger.getLogger("aprove.verification.oldframework.PropositionalLogic.Formulae.FullSharingFlatteningFactory");


    private SplitMode andMode;
    private SplitMode orMode;

    private NonCountingFormulaFactory(SplitMode andSplit, SplitMode orSplit) {
        this.andMode = andSplit;
        this.orMode = orSplit;
    }

    public static <U> NonCountingFormulaFactory<U> create(SplitMode andSplit, SplitMode orSplit) {
        return new NonCountingFormulaFactory<U>(andSplit, orSplit);
    }

    @Override
    public <U> NonCountingFormulaFactory<U> toTheory() {
        return new NonCountingFormulaFactory<U>(this.andMode, this.orMode);
    }
    ...
    public Formula<T> buildXor(List<Formula<T>> fmlae) {
        ...
        default:
            ...
            if (Globals.DEBUG_FUHS) {
                NonCountingFormulaFactory.log.log(Level.FINE, "******** Entered buildXor(...) with a list of " + size + " formulae as arg ********\n");
            }
