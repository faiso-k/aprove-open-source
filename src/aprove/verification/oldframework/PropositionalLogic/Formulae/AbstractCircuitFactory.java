...
/**
 * Provides some common functionality and attributes for CircuitFactories
 * where (multi)sets of arguments are cached.
 *
 * @author Carsten Fuhs
 */
public abstract class AbstractCircuitFactory<T> extends AbstractFormulaFactory<T> {

    private static final Logger log = Logger.getLogger("aprove.verification.oldframework.PropositionalLogic.Formulae.AbstractCircuitFactory");
    ...
    public Formula<T> buildXor(List<Formula<T>> fmlae) {
        ...
        default:
            ...
            if (Globals.DEBUG_FUHS) {
                AbstractCircuitFactory.log.log(Level.FINE, "******** Entered buildXor(...) with a list of " + size + " formulae as arg ********\n");
            }
