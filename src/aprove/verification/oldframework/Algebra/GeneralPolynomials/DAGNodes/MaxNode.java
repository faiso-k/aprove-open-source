...
public class MaxNode<C extends GPolyCoeff, V extends GPolyVar> extends BinaryNode<C, V> {
    /**
     * Create a MinusNode which connects the two given Polynomials.
     * @param leftParam The minuend.
     * @param rightParam The subtrahend.
     */
    MaxNode(final GPoly<C, V> leftParam, final GPoly<C, V> rightParam) {
    ...
    public GPoly<C, V> visit(final GPolyVisitor<C, V> gpv) {
        ...
        return gpv.caseMaxNode(this, newLeft, newRight);
    }
    ...
    public ImmutableMap<GMonomial<V>, C> getMonomials(final Semiring<C> ringC, final CMonoid<GMonomial<V>> monoid) {
        throw new UnsupportedOperationException("max nodes can not be represented as monomials");
    }
