...
/**
 * A TimesNode represents the product of two GPolys. Before the result is
 * calculated the information about the resulting polynomial is hidden in the
 * left and right children (factors).
 * @param <C> The type of the coefficients.
 * @param <V> The type of the variables.
 */
public class TimesNode<C extends GPolyCoeff, V extends GPolyVar>
    extends BinaryNode<C, V> {
    /**
     * Create a TimesNode which connects the two given Polynomials.
     * @param leftParam The first factor.
     * @param rightParam The second factor.
     */
    TimesNode(
            final GPoly<C, V> leftParam,
            final GPoly<C, V> rightParam) {
    ...
    public GPoly<C, V> visit(final GPolyVisitor<C, V> gpv) {
        ...
        return gpv.caseTimesNode(this, newLeft, newRight);
    }
    ...
    public C computeConstant(final Semiring<C> ring) {
        ...
        if (constantLeft != null) {
            ...
            if (constantRight != null) {
                return ring.times(constantLeft, constantRight);
            }
        }
        return null;
    }
