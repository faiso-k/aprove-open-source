...
/**
 * This matrix factory represents DPs, FApps and Arguments with
 * full matrices as representors.
 *
 * @author Patrick Kabasci
 * @version $Id$
 */
public class FullMatrixFactory extends BasicMatrixFactory {
    ...
    public FullMatrixFactory(Arguments arguments) {
        this.filter = arguments.filter;
        this.dimension = arguments.size;
    }

    private FullMatrixFactory(int dimension, MatrixFilter filter) {
    ...
    public MatrixFactory duplicateSelf() {
        ...
        return new FullMatrixFactory(this.dimension, this.filter);
    ...
    public Matrix createVariableMatrix(String name) {
        VarPolynomial[][] result = new VarPolynomial[this.getSize()][this.getSize()];
    ...
    private Matrix createFullMatrix(String name) {
        ...
        for (int i=0; i < this.getSize(); i++) {
            for (int j=0; j < this.getSize(); j++) {
                MatrixFilter.Filtermode res = this.getFilter().filterCoefficient(this.getSize(), i, j);
                if (res == Filtermode.FULLRANGE) {
                    SimplePolynomial simplePoly = SimplePolynomial.create(name + i + "," + j);
                    result[i][j] = VarPolynomial.create(simplePoly);
                    super.addCoefficient(simplePoly);
    ...
    public Set<VarPolyConstraint> getDPConstraints(Matrix interL, Matrix interR) {
        ...
        Set<VarPolyConstraint> constraints = new LinkedHashSet<VarPolyConstraint>(this.getSize()*this.getSize());
