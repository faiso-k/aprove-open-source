...
public abstract class TermPairDag<N extends TermPairDagNode<N>> implements DOT_Able {
     ...
     */
    protected Map<ITerm<?>, N> termAtNode;
    /**
     * list of all nodes representing a function symbol in the dag
     */
    protected List<N> functionNodes;
    /**
     * list of all nodes representing a IVariable<?> in the dag
     */
    protected List<N> variableNodes;
    /**
     * only needed for dot output
     */
    protected ITerm<?> term1;
    protected ITerm<?> term2;

    /**
     * Constructs a new dag out of the two terms
     */
    public TermPairDag(final ITerm<?> term1, final ITerm<?> term2, final SharingMode mode) {
        this.term1 = term1;
        this.term2 = term2;
        this.termAtNode = new HashMap<ITerm<?>, N>();
    ...
    abstract N createNewNode(ITerm<?> term);

    /**
     * Adds the term to the dag with full sharing, i.e. every common
     * subexpression is shared
     * @param term To be added term
     * @param father Father node of the node representing the term
     */
    protected void addTermWithFullSharing(final ITerm<?> term, final N father) {
        ...
        if (!term.isVariable() && !isInDag) {
            for (final ITerm<?> term3 : ((IFunctionApplication<?>) term).getArguments()) {
                this.addTermWithFullSharing(term3, actualNode);
            }
        }
    }

    protected void addTermWithVariableSharing(final ITerm<?> term, final N father) {
        // term is a function application
        if (!term.isVariable()) {
            final N newNode = this.createNewNode(term);
            final IFunctionApplication<?> func = (IFunctionApplication<?>) term;
            ...
            for (final ITerm<?> child : func.getArguments()) {
                this.addTermWithVariableSharing(child, newNode);
            }
        }
        // term is a IVariable<?> so we have to check if a node exists which represents this IVariable<?>
        else {
    ...
    /**
     * @return Every node which represents a IVariable<?>
     */
    ...
    /**
     * Please recognize that this method is implemented very stupid because it
     * is only used for debugging up to now!
    ...
    public String toDOT() {
        ...
        // If a term contains only one IVariable<?> draw an extra node
