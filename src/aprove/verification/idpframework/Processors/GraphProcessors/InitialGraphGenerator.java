 ...
 * @author mpluecke
 */
public class InitialGraphGenerator {

    public InitialGraphGenerator() {
    }
    ...
    public IDependencyGraph createInitialGraph(final ItpfFactory itpfFactory,
        ...
        final InitialNodesResult initials =
            InitialGraphGenerator.createInitialNodes(infRules, rewriteRules, predefinedMap, itpfFactory, polyFactory, freshNames, aborter);
        ...
        final IQTermSet cleanedQ = InitialGraphGenerator.renameQTerms(q, freshVars);
        ...
        for (final IRule rule : allRules) {
            ...
            if (lhsNode == null) {
                ...
                final Pair<Map<IVariable<?>, IVariable<?>>, Map<IVariable<?>, IVariable<?>>> lhsSubst =
                    InitialGraphGenerator.createTermSubst(freshNames, lockedVars, rule.getLeft(),
                        lhsNodeIdL);
            ...
            if (rhsNode == null) {
                ...
                final Pair<Map<IVariable<?>, IVariable<?>>, Map<IVariable<?>, IVariable<?>>> rhsSubst =
                    InitialGraphGenerator.createTermSubst(freshNames, lockedVars, rule.getRight(),
                        rhsNodeId);
            ...
            if (rule.getCondition() != null) {
                condition = InitialGraphGenerator.renameBoundVars(rule.getCondition(), rhsNodeId, itpfFactory, freshNames).applySubstitution(lhsMatcher).applySubstitution(
                    rhsMatcher);
            } else {
