...
public class CSPAEDG implements Immutable {
    ...
    protected static Logger log = Logger.getLogger("aprove.verification.dpframework.DPProblem.Utility.CSPAEDG");
     ...
     * create EDG
     */
    private CSPAEDG(CSPADPProblem cspadp, Abortion aborter) throws AbortionException {
        this.P = cspadp.getP();
        this.cspatrs = cspadp.getCSPATRS();
        this.mu = cspadp.getMu();
        this.def_tup = cspadp.getDefTup();
    ...
    public static CSPAEDG create(CSPADPProblem cspadp, Abortion aborter) throws AbortionException {
        return new CSPAEDG(cspadp, aborter);
    }
    ...
    private boolean calculateConnection(final Node<PARule> fromNode, final Node<PARule> toNode)
        ...
        this.newnr = 0;
        TRSFunctionApplication capped_t = (TRSFunctionApplication) this.getCapped(from.getRight(), this.defs, true, false, null);
        ...
        if (this.isNoncollapsingForUniv(this.S) && !this.checkSymbols(capped_t, to.getLeft())) {
            ...
            if (this.isTotallyFree(capped_t)) {
                ...
                return this.unif.areUnifiable(this.getCapped(capped_t, this.sdefs, false, true, null), to.getLeft());
            }
        }
    }
    ...
    private TRSTerm getCapped(TRSTerm t, Set<FunctionSymbol> toCap, boolean useMu, boolean renameAllVars, String sort) {
        if (t.isVariable()) {
            ...
            if (toCap.contains(troot)) {
                ...
                ImmutableSet<Integer> mus = this.getMu(troot);
                ArrayList<TRSTerm> newargs = new ArrayList<TRSTerm>();
                for (int i = 0; i < arr; i++) {
                    if (!useMu || mus.contains(Integer.valueOf(i))) {
                        newargs.add(this.getCapped(ft.getArgument(i), toCap, useMu, renameAllVars, sorts.get(i)));
                    } else {
                        newargs.add(ft.getArgument(i));
                    }
                }
    ...
    private ImmutableSet<Integer> getMu(FunctionSymbol f) {
        ImmutableSet<Integer> res = this.mu.get(f.getName());
        if (res == null) {
            res = this.mu.get(this.getDef(f).getName());
        }
        return res;
    }
    ...
    private boolean calculateConnectionPurePA(final PARule from, final PARule to) throws AbortionException {
        ...
        if (YicesChecker.callYices(PAConstraint.toSMTLIB(tmp), CSPAEDG.log, this.aborter) == YNM.NO) {
            return false;
        } else {
            return true;
        }
    ...
    public ImmutableSet<CSPADPProblem> getSCCs() {
        Set<CSPADPProblem> subSccs = new LinkedHashSet<CSPADPProblem>();
    ...
    private CSPADPProblem getSubProblem(Set<Node<PARule>> scc) {
        Set<PARule> newP = new LinkedHashSet<PARule>();
        for (Node<PARule> node : scc) {
            newP.add(node.getObject());
        }
        return CSPADPProblem.create(ImmutableCreator.create(newP), this.cspatrs, this.def_tup);
    }
