...
@NoParams
public class CSPADPSubtermProcessor extends CSPADPProcessor {

    @Override
    protected Result processCSPADP(CSPADPProblem cspadp, Abortion aborter) throws AbortionException {
        Set<PARule> p = new LinkedHashSet<PARule>(cspadp.getP());
        Set<FunctionSymbol> tups = cspadp.getTupleSymbols();
        CSSubtermModE subby = CSSubtermModE.create(cspadp.getE(), cspadp.getMu());
        ...
        Proof proof = new CSPADPSubtermProof(deletedP, this.getAfs(theProj, tups));
        CSPADPProblem newCSPADP = CSPADPProblem.create(ImmutableCreator.create(p), cspadp.getCSPATRS(), cspadp.getDefTup());
        return ResultFactory.proved(newCSPADP, YNMImplication.EQUIVALENT, proof);
    }

    private Set<PARule> getDeletedP(Set<PARule> p, Map<FunctionSymbol, Integer> proj, CSSubtermModE subby) {
    ...
    private Set<Map<FunctionSymbol, Integer>> findProj(PARule dp, Map<FunctionSymbol, Integer> p, CSSubtermModE subby) {
        TRSFunctionApplication l = dp.getLeft();
        TRSFunctionApplication r = (TRSFunctionApplication) dp.getRight();
    ...
    private Set<Map<FunctionSymbol, Integer>> findProjOne(TRSFunctionApplication l, TRSFunctionApplication r, Set<Integer> pospos, CSSubtermModE subby) {
    ...
    private Set<Map<FunctionSymbol, Integer>> findProjTwo(TRSFunctionApplication l, TRSFunctionApplication r, Set<Integer> posposl, Set<Integer> posposr, CSSubtermModE subby) {
        Set<Map<FunctionSymbol, Integer>> res = new LinkedHashSet<Map<FunctionSymbol, Integer>>();
    ...
    private static class CSPADPSubtermProof extends Proof.DefaultProof {

        private Set<PARule> deletedPairs;
        private Afs proj;

        private CSPADPSubtermProof(Set<PARule> deletedPairs, Afs proj) {
