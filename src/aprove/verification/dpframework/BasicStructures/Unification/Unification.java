 ...
 *
 * Class which contains the "Peterson-Wegman" unification algorithm.
 * Either you can check if two terms unify, or you can get an mgu if
 * such exists. It is also possible to check a problem with more than two terms
 * like (s1=?t1,...,sn=?tn). The algorithm works on a dag. This dag is optimzed because
 * common subexpressions share one node, so the dag is as small as possible
 * and the number of iterations of the algorithm is reduced.
 * If you want to have a look at the original paper be aware of the error in the pseudocode!
 * Also the paper "About the Peterson-Wegman-Algorithm" has an error in the pseudocode!
 *
 * The method unify() is much faster than getMgu(). So use unify() if are not interested
 * in the unifier.
 *
...
public class Unification implements Exportable {
     ...
     */
    public Unification(TRSTerm term1, TRSTerm term2) {
     ...
     */
    public Unification(Set<Pair<TRSTerm,TRSTerm>> termPairSet) {
        // uniform case
        if(termPairSet.size()==1){
            for(Pair<TRSTerm,TRSTerm> termPair: termPairSet) {
            ...
            FunctionSymbol f = FunctionSymbol.create("newRoot",termPairSet.size());
            int size = termPairSet.size();
            ArrayList<TRSTerm> argsTerm1 = new ArrayList<TRSTerm>(size);
            ArrayList<TRSTerm> argsTerm2 = new ArrayList<TRSTerm>(size);
            for(Pair<TRSTerm,TRSTerm> termPair: termPairSet){
                argsTerm1.add(termPair.x);
                argsTerm2.add(termPair.y);
            }
            this.term1 = TRSTerm.createFunctionApplication(f, ImmutableCreator.create(argsTerm1));
            this.term2 = TRSTerm.createFunctionApplication(f, ImmutableCreator.create(argsTerm2));
            this.dag = new UnificationDag(this.term1,this.term2);
        }
    }

    /**
     * Calls the unification algorithm if needed and returns an mgu if such exists.
    ...
    /**
     * Checks if the two given terms unify. This is much faster than running this.getMgu().
     * So use this if you do not need the mgu.
    ...
    public boolean unify() {
        ...
        for(UnificationNode funcNode: this.dag.getFunctionNodes()) {
        ...
        for(UnificationNode varNode: this.dag.getVariableNodes()) {
     ...
     */
    private boolean finish(UnificationNode r) {
        if(Globals.DEBUG_SPECIALMAN && Unification.deepDebug) {
            System.out.println("debug unif: finish with " + r);
        }
        ...
        Stack<UnificationNode> stack = new Stack<UnificationNode>();
        stack.push(r);

        while(!stack.isEmpty()) {
            UnificationNode s = stack.pop();

            if(r.isFunctionNode() && s.isFunctionNode()) {
                // check for symbol clash
                FunctionSymbol f1 = ((TRSFunctionApplication)r.getTerm()).getRootSymbol();
                FunctionSymbol f2 = ((TRSFunctionApplication)s.getTerm()).getRootSymbol();
                if(!f1.equals(f2)) {
                    // failure: Symbol clash!
                    if(Globals.DEBUG_SPECIALMAN) {
                        if(this.errorMessage.equals("")){
                            this.errorMessage = "Symbol clash occured (between " + f1.getName() + " and " + f2.getName() + ")";
                        }
                    }
                    return false;
                }
            ...
            for(UnificationNode father: s.getFathers()) {
            ...
            for(UnificationNode t: s.getEquivNodes()) {
            ...
            // if s is a variable or decompose
            if(s!=r) {
    ...
    /**
     * Because the unification algorithm creates no substitution, this
     * has to be made here using the equivalent edges in the dag which
     * exist after running the unification algorithm
     * @return the final substitution
     */
    private void buildFinalSubst() {
        this.finalSubstMap = new LinkedHashMap<TRSVariable,TRSTerm>();
        for(UnificationNode varNode: this.dag.getVariableNodes()) {
            TRSVariable actVar = (TRSVariable)varNode.getTerm();
            if(!this.finalSubstMap.containsKey(actVar)) {
                this.finalSubstMap.put(actVar,this.getFinalTermForTerm(varNode));
            }
        }
        this.mgu = TRSSubstitution.create(ImmutableCreator.create(this.finalSubstMap));
    }
    ...
    private TRSTerm getFinalTermForTerm(UnificationNode termNode) {
        ...
        if(termNode.getPointer()!=termNode) {
            ...
            // which it is representing with all variables substituted
            TRSFunctionApplication fapp = (TRSFunctionApplication)termNode.getTerm();
            FunctionSymbol f = fapp.getRootSymbol();
            ArrayList<TRSTerm> args = new ArrayList<TRSTerm>(termNode.getChildren().size());
            for (UnificationNode childNode : termNode.getChildren()) {
                args.add(this.getFinalTermForTerm(childNode));
            }
            // remember that this term has been computed
            termNode.setSubstTerm(TRSTerm.createFunctionApplication(f, ImmutableCreator.create(args)));
            return termNode.getSubstTerm();
        }
    }
    ...
    private boolean checkPropernessOfMgu() {
        if(this.mgu != null) {
            TRSTerm newTerm1 = this.term1.applySubstitution(this.mgu);
            TRSTerm newTerm2 = this.term2.applySubstitution(this.mgu);
    ...
    public String export(Export_Util eu) {
        ...
        TRSSubstitution mgu = this.getMgu();
        if(mgu == null) {
