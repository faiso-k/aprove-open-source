...
/**
 * Calculate for every pair the fan-in and fan-out in the DP graph.
 * Then only the pair with the minimal value is regarded further.
 *
...
public class MinFanHeuristic implements ReductionPairHeuristic{
    ...
    public Set<Rule> getSubset(QDPProblem qdp) {
        ...
        int fan = Integer.MAX_VALUE;
        Rule p = null;
        for (Node<Rule> node : nodeSet) {
            int newFan = graph.getIn(node).size() + graph.getOut(node).size();
            if (newFan < fan) {
                fan = newFan;
                p = node.getObject();
            }
