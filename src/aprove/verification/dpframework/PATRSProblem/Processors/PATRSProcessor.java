...
public abstract class PATRSProcessor extends Processor.ProcessorSkeleton {

    /*
     * Process a ETRS with a non-empty set of rules R
     */
    protected abstract Result processPATRS(PATRSProblem patrs, Abortion aborter) throws AbortionException;
    ...
    public Result process(BasicObligation o, BasicObligationNode oblNode, Abortion aborter, RuntimeInformation rti) throws AbortionException {
        PATRSProblem problem = (PATRSProblem) o;
        if (problem.getR().isEmpty()) {
            return ResultFactory.proved(new PATRSProcessor.RIsEmptyProof());
        } else {
            return this.processPATRS(problem, aborter);
        }
    }

    @Override
    public boolean isApplicable(BasicObligation o) {
        return (o instanceof PATRSProblem);
    }
