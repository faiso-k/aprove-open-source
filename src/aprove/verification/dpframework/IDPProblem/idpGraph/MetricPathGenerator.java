...
public class MetricPathGenerator extends AbstractPathGenerator<MetricPathGenerator.Data> {
    ...
    @Override
    protected PathDirection decidePathDirection(
            IIDependencyGraph graph, Node node,
            List<Pair<Integer, ? extends List<Node>>> paths,
        ...
        if (!data.nonCondNodes.contains(node)) {
            if (this.forceExpandCondNode && currentPath.y.size() <= 1 && !succs.isEmpty()) {
                return PathDirection.Post;
            }
            if (this.noCondPath) {
    ...
    @Override
    protected Data getInitialData(IIDependencyGraph graph, Node node) {
