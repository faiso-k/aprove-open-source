...
/**
 * Rip-off of QTermSet to obtain the symbols at the root of redexes.
 *
 * @version $Id$
 */
public final class RewriteChecker implements Immutable, HasTRSTerms, XMLObligationExportable {
     ...
     */
    public RewriteChecker(final Iterable<TRSFunctionApplication> lhss) {

        this.defSymbolsToLhs = new HashMap<FunctionSymbol, Pair<LinkedHashSet<TRSFunctionApplication>, LinkedHashSet<TRSFunctionApplication>>>();
    ...
    private void addQ(final Set<TRSFunctionApplication> allLhs, TRSFunctionApplication q) {
        ...
        while (mapIterator.hasNext()) {
            ...
            while (oldLhsIterator.hasNext()) {
                ...
                final boolean rewrite = linear ? RewriteChecker.canBeRewrittenLinear(oldLhs, q) : RewriteChecker.canBeRewrittenNonLinear(oldLhs, q, f);
                if (rewrite) {
...
*/        return this.signature;
    ...
    public boolean canAllLhsBeRewritten(final Set<Rule> R) {
        if (R.isEmpty()) {
            return true;
        }
        if (this.isEmpty) {
     ...
     * @param fs
     */
    public void collectRewritableSymbols(final TRSTerm t, final Set<FunctionSymbol> fs) {
        if (! this.isEmpty) {
            this.collectRewritableSymbolsWithNonEmptyQ(t, fs);
        }
    }

    private void collectRewritableSymbolsWithNonEmptyQ(TRSTerm t, final Set<FunctionSymbol> fs) {
        ...
        if (this.allLhs.contains(t)) {
            fs.add(((TRSFunctionApplication)t).getRootSymbol());
        }
        ...
        for (final TRSFunctionApplication subTerm : t.getNonVariableSubTerms()) {
            ...
            if (linearNonLinear != null) {
                for (final TRSFunctionApplication linear : linearNonLinear.x) {
                    if (linear.linearMatches(subTerm)) {
                        fs.add(linear.getRootSymbol());
                    }
                }
                for (final TRSFunctionApplication nonLinear : linearNonLinear.y) {
                    if (nonLinear.matches(subTerm)) {
                        fs.add(nonLinear.getRootSymbol());
                    }
                }
            }
        }
    ...
    public boolean equals(final Object other) {
        ...
        if(other instanceof RewriteChecker) {
            return this.allLhs.equals(((RewriteChecker) other).allLhs);
        }
        return false;
    }
