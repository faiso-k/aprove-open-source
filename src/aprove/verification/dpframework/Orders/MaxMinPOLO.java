...
/**
 * Polynomial orderings over the natural numbers where moreover max and min
...
public class MaxMinPOLO implements QActiveOrder, QActiveCondition.Afs {
    ...
    private static final Logger log = Logger.getLogger("aprove.verification.dpframework.Orders.MaxMinPOLO");

    final static String orderName = "Polynomial ordering with max and min";
    ...
    private MaxMinPOLO(final MaxMinPoloInterpretation interpretation,
            final Map<TermPair, OrderRelation> elements, final Abortion aborter, final boolean checkSolves) {
     ...
     * @return
     */
    public static MaxMinPOLO create(final MaxMinPoloInterpretation interpretation,
            final Map<TermPair, OrderRelation> elements, final Abortion aborter) {
        return new MaxMinPOLO(interpretation, elements, aborter, true);
    }
    ...
    private boolean checkSolves(final TRSTerm l, final TRSTerm r, final OrderRelation queriedType)
        ...
        if (condVPCs.isEmpty()) {
            ...
            if (Globals.DEBUG_FUHS && MaxMinPOLO.log.isLoggable(Level.FINEST)) {
                MaxMinPOLO.log.finest("Checking " + l + " " + queriedType + " " + r + " took " +
                    (millis2 - millis1) +
        ...
        millis2 = System.currentTimeMillis();
        if (MaxMinPOLO.log.isLoggable(Level.FINEST)) {
            MaxMinPOLO.log.finest("Checking " + l + " " + queriedType + " " + r + " took " +
    ...
    public YNM filterPosition(final FunctionSymbol f, final int i) {
        final OpVarPolynomial inter = this.interpretation.getInterpretation(f);
        if (inter == null) {
            return YNM.MAYBE;
        }
        final String x = MaxMinPoloInterpretation.VAR_PREFIX+(i+1);
