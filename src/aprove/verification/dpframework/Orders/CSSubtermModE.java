...
/**
 * Implementation of the context-sensitive subterm order modulo a collapse-free i.u.v. E.
 *
 * @author Stephan Falke
 * @version $Id$
 */

public class CSSubtermModE implements ExportableOrder<TRSTerm> {

    final static String orderName = "Context-sensitive Subterm Modulo";
    ...
    /* constructor */
    private CSSubtermModE(final ImmutableSet<Equation> E, final ImmutableMap<String, ImmutableSet<Integer>> mu) {
        this.E = E;
        this.mu = mu;
     ...
     */
    public static CSSubtermModE create(final ImmutableSet<Equation> E, final ImmutableMap<String, ImmutableSet<Integer>> mu) {
        return new CSSubtermModE(E, mu);
    }
    ...
    private boolean inRelationSub(final TRSFunctionApplication s, final TRSTerm t) {
        ...
        if (t.isVariable()) {
            result = CSTermHelper.getActiveVariables(s, this.mu).contains(t);
        } else {
            final int arr = s.getRootSymbol().getArity();
            for (int i = 0; i < arr; i++) {
                if (this.mu.get(s.getRootSymbol().getName()).contains(Integer.valueOf(i))) {
    ...
    public String toString() {
        return "Context-sensitive Subterm Modulo";
    }
    ...
    public String export(final Export_Util o) {
        return CSSubtermModE.orderName;
    }
