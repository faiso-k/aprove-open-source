...
/** Implementation of the Recursive Path Order with a non-strict precedence.
 *  <p>
...
public class QRPO extends AbstractQuasiPO implements MultisetExtensibleOrder<TRSTerm> {

    final static String orderName = "Recursive Path Order with Non-Strict Precedence";

    private final HashOrder ho;

    /* constructors */

    private QRPO(final Qoset<FunctionSymbol> precedence) {
    this.precedence = precedence;
    this.ho = HashOrder.createHO();
    }

    /** Creates a new instance of <code>QRPO</code>.
     * @param signature   the names of the symbols
     * @param precedence   the precedence to be used
     */
    public static QRPO create(final Qoset<FunctionSymbol> precedence) {
    return new QRPO(precedence);
    }

    /** Calculates the minimal extensions of <code>q</code> such that
     * <code>s</code> and <code>t</code> are quasi-equal as multiterms.
     * @param s   a term
    ...
    return QRPO.minimalExt(s, t, q, eq, true);
    }
    ...
    return QRPO.minimalExt(s, t, q, eq, false);
    }
    ...
    if(Multiterm.create(origS, q).equals(Multiterm.create(origT, q))) {
        ...
        final DoubleHash<TRSTerm,TRSTerm,ExtHashSetOfQosets<FunctionSymbol>> dh = DoubleHash.create();
        TRSTerm newLeft;
        TRSTerm newRight;
        ...
        while(j.hasNext()) {
            ...
            dh.put(newLeft, newRight, QRPO.minimalExt(newLeft, newRight, tmp, eq, equal));
        }
        }
        ...
        ExtHashSetOfQosets<FunctionSymbol> tmpRes = ExtHashSetOfQosets.create(q.getSet());
        tmpRes.add(tmp);
        ...
        while(i.hasNext()) {
            ...
            res = ExtHashSetOfQosets.create(q.getSet());
            return res;
            }
        }
        try {
    ...
    if(Multiterm.create(origS, this.precedence).equals(Multiterm.create(origT, this.precedence))) {
        /* the terms already are quasi-equal */
        return true;
    }
    else if(origS.isVariable()) {
        ...
        while(i.hasNext() && result) {
            result = dh.get((TRSTerm)i.next(), (TRSTerm)j.next());
        }
                if (result) {
                    break;
                }
     ...
     *                       {s_1, ... , s_n} >> {t_1, ... , t_m}
     *                       where >> is the multiset extension and f and
     *                       g are equivalent
     *                   or
     ...
     * are equal as multiterms up to equivalence of function symbols.
     */
    @Override
    public boolean areEquivalent(final TRSTerm s, final TRSTerm t) {
    return Multiterm.create(s, this.precedence).equals(Multiterm.create(t, this.precedence));
    }
    ...
    /**
     * @return the implicit status map of this QRPO -- compare arguments
     *  as multisets
     */
    @Override
    ...
    public String export(final Export_Util eu) {
        return "Recursive Path Order "+eu.cite(Citation.RPO)+"."+eu.linebreak()+
