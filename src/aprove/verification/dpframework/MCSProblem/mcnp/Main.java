    ...
    public static void testFolder(String dir, boolean verify) {
        ...
        for (int i=0; i<files.length; i++) {
            ...
            try {
                ...
                Program pr = Program.create(files[i].getAbsolutePath());
                pr.findLevelMappings();
                List<MCGraphMapping> rankingFunction = pr.getMcGraphsMappings();
                long time = System.currentTimeMillis()-startTime;
                totalTime+=time;
                String timeReport = "["+time/60000+":"+(time/1000)%60+" ("+time+" milisec)]";
        ...
        String totalTimeReport = "["+totalTime/(60*60*1000)+":"+(totalTime/(60*1000))%60+":"+(totalTime/1000)%60+" ("+totalTime+" milisec)]";
...
//        if (1==1) return;
        ...
        Program pr = Program.create(args[0]);
        pr.findLevelMappings();
        List<MCGraphMapping> rankingFunction = pr.getMcGraphsMappings();
