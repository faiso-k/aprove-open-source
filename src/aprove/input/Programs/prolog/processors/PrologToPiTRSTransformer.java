...
import aprove.input.Programs.prolog.processors.PrologToPiTRSTransformer.AfsModeAnalyser.ModedRule.*;
...
/**
 * Converts a PrologObligation into into the corresponding TRS Obligation. If
 * the PrologProgram in the obligation is no logic program, the processor is not
 * applicable.<br>
 * <br>
 * Created: Jul 28, 2006<br>
 * Last modified: Nov 14, 2006
 * @author cryingshadow
 * @version $Id$
 */
public class PrologToPiTRSTransformer extends AbstractPrologToTRSTransformer {
    ...
    // Just for PrologTest
    public PrologToPiTRSTransformer() {
        this(new Arguments());
    }

    @ParamsViaArgumentObject
    public PrologToPiTRSTransformer(final Arguments arguments) {
        this.force = arguments.force;
        this.heuristic = arguments.heuristic;
    }

    public
        Pair<PiTRSProblem, Proof>
        calculatePiTRSProblem(final PrologProblem pp, final Afs pi, final PrologFNG fridge)
    {
        final Pair<BasicObligation, Proof> p = this.calculateTRSProblem(pp, pi, fridge);
        return new Pair<PiTRSProblem, Proof>((PiTRSProblem) p.x, p.y);
    }
        ...
        calculateTRSProblem(final PrologProblem pp, final Afs pi, final PrologFNG fridge)
    {
        ...
        Set<GeneralizedRule> R =
            AbstractPrologToTRSTransformer.translate(
                ...
                fridge,
                this.identify,
                this.eliminate).x;//System.err.println(Pi);
        ...
        R = pair.x;
        Pi = pair.y.reduceToSignature(CollectionUtils.getFunctionSymbols(R));
        //            Set<FunctionSymbol> sig = Collections.getFunctionSymbols(R);
        //            Pi = Pi.reduceToSignature(sig);
        final PiTRSProblem pitrs = PiTRSProblem.create(ImmutableCreator.create(R), new ImmutableAfs(Pi));
        return
            new Pair<BasicObligation, Proof>(
                pitrs,
                new PrologToPiTRSProof(
    ...
    public Pair<Afs, String> moding(final Pair<PrologProgram, Set<PrologQuery>> obl) {
        if (obl.y.size() != 1) {
            return new Pair<Afs, String>(null, "Expecting exactly one query!");
        }
        ...
        final PrologProgram prologProg = obl.x;
        prologProg.flattenOutConjunctions();

        if (!PrologProgram.isLogicProgram(prologProg)) {
            return new Pair<Afs, String>(null, "The Prolog program is no logic program.");
        }
        ...
        Afs pi = piList.iterator().next();
        final Set<ConditionalRule> conditionals = new LinkedHashSet<ConditionalRule>();

        final PrologFNG fridge = prologProg.createFreshNameGenerator();
        for (final PrologClause clause : prologProg.getClauses()) {
            conditionals.add(AbstractPrologToTRSTransformer.toConditionalRule(clause, fridge));
        }
        ...
        final Triple<Set<GeneralizedRule>, Afs, Map<FunctionSymbol, Set<List<Boolean>>>> pair =
            new AfsModeAnalyser(R, pi, heuristic, this.force).getSplitRules();
        pi = pair.y.reduceToSignature(CollectionUtils.getFunctionSymbols(pair.x));
        final Afs afs = new Afs();
        for (final Triple<FunctionSymbol, YNM[], Boolean> entry : pi.getFilterings()) {
            final FunctionSymbol f = entry.x;
            final int arity = f.getArity();
            if (arity == 0) {
                continue;
            }
            final String[] names = f.getName().split("_", -1);
            final YNM[] filtering = entry.y;
            final int len = names.length;
            if (len > 2 && names[len - 2].equals("in") && this.isGA(names[len - 1])) {
                // inSymbol
                String name = names[0];
                for (int i = 1; i < len - 2; i++) {
                    name += "_" + names[i];
                }
                final FunctionSymbol orig = FunctionSymbol.create(name, arity);
                if (!afs.hasFilter(orig)) {
                    afs.setNoFiltering(orig);
                }
                for (int i = 0; i < arity; i++) {
                    if (filtering[i] == YNM.NO) {
                        afs.setFiltering(orig, i, YNM.NO);
                    }
                }
            } else if (len > 2 && names[len - 2].equals("out") && this.isGA(names[len - 1])) {
                // out-syymbol -> do nothing
            } else if (len == 2 && this.isUDigit(names[0]) && this.isGA(names[1])) {
                // U-symbol -> do nothing
            } else {
                // normal symbol
                afs.setFiltering(f, filtering);
            }
        }
        return new Pair<Afs, String>(afs, null);
    }

    private boolean isGA(final String s) {
    ...
    private boolean isUDigit(final String s) {
        if (s.length() == 0 || s.charAt(0) != 'U') {
            return false;
        }
    ...
    public static void extendPi(final Afs Pi, final Set<GeneralizedRule> R) {
        ...
        while (tryToExtend) {
            ...
            for (final GeneralizedRule rule : R) {
                tryToExtend = PrologToPiTRSTransformer.extendPi(Pi, rule) || tryToExtend;
            }
        }
    }
    ...
    private static boolean extendPi(final Afs Pi, final GeneralizedRule rule) {
        ...
        if (!evilVars.isEmpty()) {
            ...
            return PrologToPiTRSTransformer.extendPi(Pi, (TRSFunctionApplication) rule.getRight(), evilVars);
        }
        return false;
    }
        ...
        for (int i = 0; i < regarded.length; i++) {
            if (regarded[i]) {
                ...
                if (argi.isVariable()) {
                    ...
                    changed = PrologToPiTRSTransformer.extendPi(Pi, (TRSFunctionApplication) argi, evilVars) || changed;
                    newArgs[i] = YNM.YES;
                }
            } else {
                newArgs[i] = YNM.NO;
    ...
    public class PrologToPiTRSProof extends Proof.DefaultProof {
        ...
        public PrologToPiTRSProof(
            final PrologProblem pp,
            final PiTRSProblem pitrs,
            final Set<GeneralizedRule> R,
        ...
        public String export(final Export_Util o, final VerbosityLevel level) {
            ...
            this.result.append("We use the technique of "
                + o.cite(Citation.TOCL09)
                + ". With regard to the inferred argument filtering the"
            ...
            //            if (true || this.R == null) { // no CTRS to TRS conversion was necessary
            this.result.append("Transforming "
                ...
                + o.linebreak());
            //            } else {
            //                this.result.append("Transforming " + ProofUtility.R(o, this.pp.getName(NameLength.SHORT)) + " into the following " + ProofUtility.CTRS(o) + ":\n" + o.linebreak());
            //                this.result.append(o.export(this.R));
            //                this.result.append("\n" + o.paragraph());
            //                this.result.append("and onward into the following  " + ProofUtility.TRS(o) + " " + this.pitrs.getName(aprove.verification.dpframework.NameLength.SHORT) + ":\n" + o.linebreak());
            //            };

            this.result.append(o.export(this.pitrs));
            this.result.append("\n" + o.paragraph());

            this.result.append(o.bold("Infinitary Constructor Rewriting Termination")
                + " of "
                + this.pitrs.getName(aprove.verification.dpframework.NameLength.SHORT)
                + " implies "
                + o.bold("Termination")
                + " of "
    ...
    public static class Arguments {
        ...
        public PrologToPiTRSTransformer.Heuristic heuristic = PrologToPiTRSTransformer.Heuristic.IM2;
    }
