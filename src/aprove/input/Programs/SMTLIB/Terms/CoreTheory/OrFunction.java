...
public class OrFunction extends AbstractFunctionWrapper implements Immutable {
    private static final VariadicFunctionSort OrType =
        VariadicFunctionSort.create(SortBool.SORTBOOL, SortBool.SORTBOOL, 2);

    public OrFunction(final FormulaFactory<Diophantine> formulaFactory) {
        super(OrFunction.OrType, formulaFactory);
    }
    ...
    public SMTTermWrapper realApply(final List<SMTTermWrapper> arguments)
        ...
        return new DiophantineFormulaWrapper(
            this.formulaFactory.buildOr(fmlae), this.formulaFactory);
    }
}
