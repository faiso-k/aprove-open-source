...
public class PlusFunction extends AbstractFunctionWrapper implements Immutable {
    private static final VariadicFunctionSort PlusType =
        VariadicFunctionSort.create(SortInt.SORTINT, SortInt.SORTINT, 2);

    public PlusFunction(final FormulaFactory<Diophantine> formulaFactory) {
        super(PlusFunction.PlusType, formulaFactory);
    }
    ...
    public SMTTermWrapper realApply(final List<SMTTermWrapper> arguments)
        ...
        for (final SMTTermWrapper t : arguments) {
            if (p == null) {
                ...
                p = p.plus(t.getSimplePolynomial());
            }
        }
