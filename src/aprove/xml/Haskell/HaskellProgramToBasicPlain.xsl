...
<xsl:template match="Term[count(Apply/Term[1]/Apply/Term[1]/Cons[@infix]/Arrow)>0]" priority ="5">
    <xsl:param name = "Second" />
    <xsl:param name = "Priority" />
    <xsl:param name = "Fixity" />
    <xsl:param name = "Pos" />
...
<xsl:template match="MainModule">
   ...
   <table cellspacing="0" cellpadding="0" border="0" frame ="void" >
        <xsl:for-each select="StartTerm">
                <xsl:text>{-# htermination </xsl:text>
		<xsl:apply-templates select="." />
                <xsl:text> #-} &#10;</xsl:text>
	</xsl:for-each>
   </table>
   ...
   <br/>
   <xsl:text>import qualified Prelude &#10;</xsl:text>
   <xsl:text>data MyBool = MyTrue | MyFalse &#10;</xsl:text>
   <xsl:text>data List a = Cons a (List a) | Nil &#10;</xsl:text>
</xsl:template>
