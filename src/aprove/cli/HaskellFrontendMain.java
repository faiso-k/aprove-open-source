...
public class HaskellFrontendMain extends FrontendMain<HaskellFrontendMain.HaskellFrontendOpts> {
    /**
     * Record-like class holding all options for this frontend.
     */
    static class HaskellFrontendOpts extends FrontendMain.FrontendOpts {
        /** JSON Graph export. */
    ...
    @Override
    HaskellFrontendOpts createFrontendOpts() {
        return new HaskellFrontendOpts();
    }
    ...
    Getopt createGetopt(List<LongOpt> longoptsArg, String[] argv) {
        ...
        return new Getopt("AProVE Haskell Frontend", argv, "ho:t:ps:g:j:", longopts.toArray(new LongOpt[0]));
    }
    ...
    String getStrategyName() {
        if (this.options.graphExport) {
            return "aprove.FrontendExport.HaskellFrontend-graph";
        } else if (this.options.jsonExport) {
            return "aprove.FrontendExport.HaskellFrontend-json";
        } else {
            return "aprove.FrontendExport.HaskellFrontend-trs";
        }
    }
    ...
    @Override
    void printHelp() {
        System.out.println("Usage: java -cp aprove.jar aprove.CommandLineInterface.HaskellFrontendMain [OPTION] HSFILE\n");
        System.out.println("Haskell to QDP/Graph dump export from AProVE 2014.");
        System.out.println();
    ...
    public static void main(String args[]) {
        try {
            new HaskellFrontendMain().run(args);
